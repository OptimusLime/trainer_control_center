...
```
  ...
  ┌──────────────────────────────────────┐
  │  Browser ──> UI Process (:8081)      │
  │              │                        │
  │              │ ACC_TRAINER_URL =      │
  │              │ http://paul-cheddar:6060│
  └──────────────┼────────────────────────┘
                 │ Tailscale
  ┌──────────────┼────────────────────────┐
  │  Trainer Process (:6060)             │
...
1. `trainer_main.py` binds to `0.0.0.0:6060` — already listens on all interfaces
2. `ui_main.py` binds to `0.0.0.0:8081` — already listens on all interfaces
3. `ui/app.py` reads `ACC_TRAINER_URL` from env var (default `http://localhost:6060`)
...
**Outcome:** I can start the UI with `python -m acc.ui_main --trainer-url http://paul-cheddar:6060`
...
5. Verify: start trainer, start UI with `--trainer-url http://localhost:6060`, dashboard loads
...
4. Dashboard shows "Connected to http://localhost:6060"
...
10. Dashboard loads, health green, shows "Connected to http://paul-cheddar:6060"
...
- `--remote` — shell script flag for the common case (expands to `--trainer-url http://<host>:6060`)
